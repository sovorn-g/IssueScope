...
const sampleIssues: UiIssue[] = [
  { sev: "Critical", title: "Install fails on Node 22 with native binding error", no: 1287, body: "When running `npm install` on Node.js 22.11.0 (Linux x64 / macOS arm64), the native binding build step exits with:\n\n```\nError: Cannot find module './build/Release/binding.node'\n    at Object.<anonymous> (/node_modules/scope-core/dist/index.js:12:18)\n```\n\n**Steps to reproduce:**\n1. Fresh clone `npx create-next-app@latest`\n2. `npm install`\n3. Observe error above\n\n**Environment:**\n- OS: Ubuntu 24.04 / macOS 15\n- Node: 22.11.0\n- npm: 10.9.0", labels: ["bug", "install"], comments: 19, updated: "18m ago", repro: false, dupes: 4, reason: "Blocks fresh installs and has multiple confirmations across Linux and macOS.", dup: [[1268, "Native binding cannot load after 2.4.0", ".91"], [1241, "Postinstall exits 1 on CI", ".84"]] },
  { sev: "High", title: "Hydration mismatch after upgrading to React 19", no: 1279, body: "After upgrading from React 18.3 to 19.0, pages using `useSearchParams()` in the root layout throw hydration warnings:\n\n```\nWarning: Text content did not match. Server: \"\" Client: \"?page=1\"\n```\n\nThe issue appears when the layout is wrapped in a Suspense boundary. Wrapping only the component that consumes search params works, but the root-level pattern that worked in 18.x is now broken.", labels: ["bug", "regression"], comments: 8, updated: "2h ago", repro: true, dupes: 3, reason: "Affects upgrade path and includes production impact but no minimal reproduction.", dup: [[1252, "Client/server mismatch on nested route", ".87"], [1220, "React 19 warning on first render", ".81"]] },
  { sev: "High", title: "CLI reports success but leaves config file empty", no: 1273, body: "Running `npx scope init --template default` prints:\n\n```\n✔ Config written to scope.config.json\n```\n\nBut `scope.config.json` is an empty file (0 bytes). This happens only when the target directory already contains a `.git` folder but no prior config. On a fresh directory it works correctly.", labels: ["cli", "data-loss"], comments: 5, updated: "4h ago", repro: true, dupes: 1, reason: "Potential data-loss workflow with incomplete environment details.", dup: [[1194, "Init command overwrites config", ".79"]] },
  { sev: "Medium", title: "Windows path separator breaks generated routes", no: 1266, body: "On Windows 11 (PowerShell 7), dynamic route generation produces backslash-prefixed paths like `\\users\\[id]` instead of `/users/[id]`. This causes 404s on all dynamic routes. The issue is in the `normalizePath` utility which uses `path.resolve` without converting back to forward slashes.", labels: ["windows"], comments: 4, updated: "1d ago", repro: false, dupes: 0, reason: "Platform-specific breakage with clear reproduction steps.", dup: [] },
];
...
export default function Home() {
  ...
  const summary = useMemo(() => [
    ...
    ["Likely duplicates", String(buildDuplicateGroups(issues).length), "Distinct issue clusters sharing root causes."],
  ], [issues]);
